...
SqlSelectQueryBuilder& SqlSelectQueryBuilder::Add_Where(const std::map<std::string, std::string>& kv) noexcept
{
	for (auto& [key, value] : kv)
	{
		Id_Name.emplace(std::pair<std::string,std::string>(key,value));
	}
	return *this;
}
